...
instance SpaceSize GymSpace where
  ...
  spaceSize (Box _ (Gym1D x1))    = x1
  spaceSize (Box _ (Gym2D x1 x2)) = x1 * x2
  spaceSize (Tuple spaces)        = product (map spaceSize spaces)


toAction :: GymSpace -> Integer -> IO Py.SomeObject
toAction (Discrete nr) idx | idx < 0 || idx > (nr-1) = error "Action index out of range"
                           | otherwise = Py.toObject <$> Py.toInteger idx
toAction (Box _ (Gym1D x1)) idx = error "Box (probably continuous) actions currently not supported"
toAction (Box _ (Gym2D x1 x2)) idx = error "Box (probably continuous) actions currently not supported"
